...
リファクタリングと並行して、以下のパフォーマンス最適化を実施する。

| 対象 | 作業内容 | 期待効果 |
|------|---------|----------|
| `style.css` | 未使用CSSルールの削除 | 18 KiB削減 |
| `style.css` | CSS圧縮（minify） | 2 KiB削減 |
| `app.js` | 未使用コードの削除 | 82 KiB削減 |
| `app.js` | JavaScript圧縮（minify） | 8 KiB削減 |
| `index.html` | レンダリングブロックリソースの最適化 | FCP改善 |
| `index.html` | クリティカルCSSのインライン化検討 | LCP改善 |

**注意**: 圧縮（minify）は開発時は実施せず、本番デプロイ時のビルドプロセスで対応することを推奨。リファクタリングでは主に「未使用コード削除」と「構造最適化」を実施する。
